...
for(i in 1:nrow(sim_candidates)){
  bigfolder <- "//pwdoows/oows/Watershed Sciences/GSI Monitoring/06 Special Projects/34 PWDGSI metrics calculations/EAP10/20211117_post-purge/"
  ...
  print(paste("Calccing", sim_candidates$smp_id[i]))
  ...
  print(paste("Calculating", sim_candidates$smp_id[i]))
  ...
  obs_summary <- obs_data %>%
    ...
    dplyr::summarize(
      ...
      percentstorageused_relative = marsPeakStorage_percent(waterlevel_ft = level_ft - dplyr::first(level_ft), 
      ...
      orifice_volume_cf = round(sum((orifice_outflow_ft3))),
  ...
  obs_summary <- left_join(obs_summary, depths)
  
  write_csv(obs_summary, file = paste0(bigfolder, "/", snapshot$smp_id, "_", snapshot$ow_suffix, ".csv"))
  
  # for(j in 1:length(obs_summary$radar_event_uid)){
  # 
  #   #filter for each event
  #   selected_event <- obs_data %>%
  #     dplyr::filter(radar_event_uid == obs_summary$radar_event_uid[j])
  # 
  #   rain_plot_data <- monitoringdata[["Rainfall Data"]] %>%
  #     dplyr::filter(radar_event_uid == obs_summary$radar_event_uid[j])
  # 
  #   #only plot rain events greater than 1.5". this can be modified as desired
  #   if(obs_summary$eventdepth_in[j] >= 1){
  # 
  #     #plot observed and simulated data
  #     plot <- marsCombinedPlot(event = obs_summary$radar_event_uid[j],
  ...
  # 
  #     ggplot2::ggsave(paste0(folder, "/", paste(sim_candidates$smp_id[i], sim_candidates$ow_suffix[i], obs_summary$radar_event_uid[j], sep = "_"),".png"), plot = plot, width = 10, height = 8)
  # 
  #   }
  # }
}
...
write_infiltration <- marsWriteInfiltrationData(con = mars,
                                              infiltration_rate_inhr = metrics$infiltration_inhr,
                                              ow_uid = metrics$ow_uid,
                                              radar_event_uid = metrics$radar_event_uid,
                                              snapshot_uid = metrics$snapshot_uid,
                                              observed_simulated_lookup_uid = metrics$observed_simulated_lookup_uid)
