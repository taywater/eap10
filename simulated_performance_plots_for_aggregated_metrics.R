...
# 1.0 queries and plots
  ...
  long_term_smp_metrics <- dbGetQuery(mars, "select s.ow_uid, s.smp_id, s.ow_suffix, s.asset_type, s.lined, s.surface, s.infiltration_rate_inhr, s.rel_percentstorage, s.draindown_hr, s.dd_assessment_lookup_uid, s.overtopping, s.error_lookup_uid, s.rainfall_radarcell_event_uid, s.eventdepth_in, s.eventpeakintensity_inhr, s.eventdepth_lookup_uid, s.designdepth_in, s.relative_eventdepth_lookup_uid, e.eventdepth_range_in, e.eventdepth_description, r.relative_eventdepth_range_in, r.relative_eventdepth_description from performance.summary_ow_event_radarcell s
                                             ...
                                             from fieldwork.deployment_full where term = 'Long') and s.surface = false and s.observed_simulated_lookup_uid = 2")
  give.n <- function(x){
  ...
  for(i in 1:length(unique(long_term_smp_metrics$ow_uid))){
    ...
    smp_rpsu_event_depth_plot <- ggplot(long_term_smp_metrics_select, 
        ...
        ylab("Sim Relative Percent of Storage Used") + 
    ...
    ggsave(filename = paste0(folder,date, type, smp_id, "_", ow_suffix, "_event_depth_RPSU_sim.jpg"), plot = smp_rpsu_event_depth_plot, width = 10, height = 8, units = "in")
    ...
    smp_draindown_event_depth_plot <- ggplot(long_term_smp_metrics_select, 
      ...
      geom_boxplot() +
      xlab("Event Depth Range (in)") +
      ylab("Sim Draindown (hr)") + 
      ggtitle(paste(smp_id, ow_suffix, "|", deployment_time, paste0("Days (", round(deployment_time/30, 1)), "Months) Monitored")) +
      stat_summary(fun.data = give.n, geom= "label", size = 1/4*text_size, label.size = NA) +
      theme(text = element_text(size = text_size))
    
    ggsave(filename = paste0(folder,date, type, smp_id, "_", ow_suffix, "_event_depth_draindown_sim.jpg"), plot = smp_draindown_event_depth_plot, width = 10, height = 8, units = "in")
    ...
    ggsave(filename = paste0(folder,date, type, smp_id, "_", ow_suffix, "_relative_event_depth_RPSU_sim.jpg"), plot = smp_rpsu_rel_plot, width = 10, height = 8, units = "in")
    ...
    ggsave(filename = paste0(folder,date, type, smp_id, "_", ow_suffix, "_relative_event_depth_draindown_sim.jpg"), plot = smp_draindown_rel_plot, width = 10, height = 8, units = "in")
  ...
  subsurface_metrics <- dbGetQuery(mars, "select s.ow_uid, s.smp_id, s.ow_suffix, s.asset_type, s.lined, s.surface, s.infiltration_rate_inhr, s.rel_percentstorage, s.draindown_hr, s.dd_assessment_lookup_uid, s.overtopping, s.error_lookup_uid, s.rainfall_radarcell_event_uid, s.eventdepth_in, s.eventpeakintensity_inhr, s.eventdepth_lookup_uid, s.designdepth_in, s.relative_eventdepth_lookup_uid, e.eventdepth_range_in, e.eventdepth_description, r.relative_eventdepth_range_in, r.relative_eventdepth_description from performance.summary_ow_event_radarcell s
                                              left join performance.eventdepth_bin_lookup e on e.eventdepth_lookup_uid = s.eventdepth_lookup_uid
                                              left join performance.relative_eventdepth_bin_lookup r on r.relative_eventdepth_lookup_uid = s.relative_eventdepth_lookup_uid
                                              where s.surface = false and s.observed_simulated_lookup_uid = 2")
  ...
  for(i in 1:length(unique(subsurface_metrics$asset_type))){
    ...
    ggsave(filename = paste0(folder,date, type, asset_type, "_event_depth_rpsu_sim.jpg"), plot = asset_rpsu_event_depth_plot, width = 10, height = 8, units = "in")
    ...
    #   theme(text = element_text(size = text_size))
    # 
    # ggsave(filename = paste0(folder,date, type, asset_type, "_event_depth_infiltration_sim.jpg"), plot = asset_infiltration_event_depth_plot, width = 10, height = 8, units = "in")
    ...
    ggsave(filename = paste0(folder,date, type, asset_type, "_event_depth_draindown_sim.jpg"), plot = asset_draindown_event_depth_plot, width = 10, height = 8, units = "in")
    ...
    ggsave(filename = paste0(folder,date, type, asset_type, "_relative_event_depth_rpsu_sim.jpg"), plot = asset_rpsu_rel_plot, width = 10, height = 8, units = "in")
    ...
    ggsave(filename = paste0(folder,date, type, asset_type, "_relative_event_depth_draindown_sim.jpg"), plot = asset_draindown_rel_plot, width = 10, height = 8, units = "in")
  ...
  for(i in 1:length(unique(subsurface_metrics$lined))){
    ...
    ggsave(filename = paste0(folder,date,type, "Subsurface_", lined_type, "_event_depth_rpsu_sim.jpg"), plot = lined_rpsu_event_depth_plot, width = 10, height = 8, units = "in")
    ...
    ggsave(filename = paste0(folder,date, type, "Subsurface_", lined_type, "_event_depth_draindown_sim.jpg"), plot = lined_draindown_event_depth_plot, width = 10, height = 8, units = "in")
    ...
    ggsave(filename = paste0(folder,date, type, "Subsurface_", lined_type, "_relative_event_depth_draindown_sim.jpg"), plot = lined_draindown_rel_plot, width = 10, height = 8, units = "in")
