...
for(i in 1:length(ow_testing_list$start_date)){
  ...
  if(length(rain_event_data$rainfall_radarcell_event_uid) > 0){
    ...
    folder <- (paste0("O:/Watershed Sciences/GSI Monitoring/06 Special Projects/34 PWDGSI metrics calculations/EAP10/20210520/subsurface_unlined/", paste(target_id, ow_suffix, sep = "_")))
    ...
    rain_observed_summary <- left_join(observed_summary, rain_event_data, by = "rainfall_radarcell_event_uid")
